...
# ═══════════════════════════════════════════════════
# 🤖 دستیار فرمان داخلی کامل (بدون AI / بدون اینترنت)
# این دستیار فقط روی امکانات موجود همین فایل عمل می‌کند.
# ═══════════════════════════════════════════════════
_PERSIAN_DIGITS = str.maketrans("۰۱۲۳۴۵۶۷۸۹", "0123456789")
_ASSISTANT_NUM_WORDS = {
    "صفر":0,"یک":1,"یکی":1,"دو":2,"سه":3,"چهار":4,"پنج":5,"شش":6,
...
_ASSISTANT_ACTION_WORDS = (
    "بساز","ساخت","درست کن","ایجاد کن","اضافه کن","حذف کن","پاک کن","برکنار",
    "ویرایش","تغییر","تنظیم","فعال کن","روشن کن","خاموش کن","غیرفعال کن",
    "ریست","بازنشانی","بانک","عنوان","فوتر","هدر","نقل قول","کپی کن","کپی"
)
...
def _assistant_number(value):
    ...
    if parts and all(x in _ASSISTANT_NUM_WORDS for x in parts):
        ...
        for x in parts:
            ...
            total = total*100 if n==100 and total else total+n
        return total
    return None
...
def _assistant_find_template(name):
    target=_assistant_norm(name).strip("«»\"'")
    for k in ALL_TEXTS:
        if _assistant_norm(k)==target:
            return k
    return None

def _assistant_find_section(name):
    target=_assistant_norm(name).strip("«»\"'")
    for g in TEMPLATE_GROUPS:
        if _assistant_norm(section_title(g))==target or _assistant_norm(g)==target:
            return g
        if _assistant_norm(g.split("|",1)[0])==target and not g.split("|",1)[1]:
            return g
    return None

def _assistant_template_names(section_name,count,raw_names=""):
    count=max(1,min(int(count),1000)); raw_names=_assistant_norm(raw_names)
    if raw_names:
    ...
    # اگر قبلاً وطن_01 تا وطن_20 ساخته شده باشند، از 21 ادامه می‌دهد.
    used=[]
    ...
    return [f"{base}_{i:02d}" for i in range(start,start+count)]

def _assistant_bank_items(payload):
    payload=re.sub(r"^(?:متن(?:های)?|آیتم(?:ها)?|عنوان(?:ها)?)\s*[:：]\s*", "", payload.strip())
    return [x.strip(" \t\n،,؛;") for x in re.split(r"\n+|\s*\|\s*|\s*؛\s*",payload) if x.strip()]

def _assistant_title_lines(payload):
    return [x.strip() for x in str(payload or "").splitlines() if x.strip()]

def _assistant_extract_name_after(text, patterns):
    for p in patterns:
        m=re.search(p,text,re.I|re.S)
        if m: return _assistant_clean_name(m.group(1))
    return ""

def _assistant_parse(text):
    raw=str(text or "").strip()
    n=_assistant_norm(raw)
    low=n.lower()
    if not n:
        return None
    if low in {"کمک","راهنما","help","دستیار کمک"}:
        return [{"type":"help"}]
    if low in {"خروج","خروج از دستیار","بستن دستیار","لغو دستیار"}:
        return [{"type":"exit"}]

    actions=[]
    # بخش + قالب در یک پیام
    sec=re.search(r"(?:^|[،,])\s*(?:یک\s+)?(?:بخش|پوشه)\s+(?:به\s+)?(?:اسم|نام|به\s+نام)?\s*[:：]?\s*[«\"']?([^«»\"'،,؛;]+?)[»\"']?\s*(?=\s*(?:بساز|درست کن|ایجاد کن))",n,re.I)
    if sec:
        name=_assistant_clean_name(sec.group(1))
        if name: actions.append({"type":"create_section","name":name})

    # ساخت قالب‌ها؛ هم «۲۰ تا قالب برای ل» و هم «برای ل، ۲۰ قالب»
    tm=re.search(r"(?:([0-9]+|[آ-ی]+)\s*)?(?:تا\s*)?(?:قالب|قالبها|قالب ها)\s+(?:برای|داخل|تو|در)\s+(?:بخش\s+|پوشه\s+)?[«\"']?([^«»\"'،,؛;]+?)[»\"']?(?=\s*(?:بساز|درست کن|ایجاد کن|با\s+نام|با|،|$))",n,re.I)
    if not tm:
        tm=re.search(r"(?:برای|داخل|تو|در)\s+(?:بخش\s+|پوشه\s+)?[«\"']?([^«»\"'،,؛;]+?)[»\"']?\s*(?:،|,)\s*(?:([0-9]+|[آ-ی]+)\s*)?(?:تا\s*)?(?:قالب|قالبها|قالب ها)",n,re.I)
    # «۲۰ تا قالب داخلش ...» به بخش ساخته‌شده در همان پیام اشاره می‌کند.
    if not tm:
        inside=re.search(r"(?:([0-9]+|[آ-ی]+)\s*)?(?:تا\s*)?(?:قالب|قالبها|قالب ها)\s+داخلش",n,re.I)
        if inside:
            inferred = sec.group(1).strip() if sec else ""
            tm=(inside, ) if inferred else None
            if tm:
                class _M:
                    def __init__(self,g1,g2,g3): self._g=(g1,g2,g3)
                    def group(self,i): return self._g[i-1]
                tm=_M(inside.group(1), inferred, None)
    if tm:
        explicit_count = _assistant_number(tm.group(1)) if tm.group(1) is not None else _assistant_number(tm.group(2))
        if tm.group(1) is not None:
            count=explicit_count; section=tm.group(2)
        else:
            section=tm.group(1); count=explicit_count
        count=count or 1; section=_assistant_clean_name(section); names=""
        nm=re.search(r"(?:با|به)\s+(?:این\s+)?(?:نام|اسم)\s*[:：]?\s*(.+?)(?=\s*(?:بساز|درست کن|ایجاد کن|و\s+پست|و\s+بانک|پست سریع|داخل بانک|$))",n,re.I)
        if nm: names=nm.group(1).strip()
        if explicit_count is not None:
            actions.append({"type":"create_templates","section":section,"count":count,"names":names})

    # بانک مشترک برای همه قالب‌های یک بخش
    bm=re.search(r"(?:داخل\s+بانک|بانک)\s+(?:همه\s+)?(?:قالب(?:ها|های| ها| های)?\s*)?(?:بخش|برای|در|داخل)?\s*[«\"']?([^:：\n]+?)[»\"']?\s*[:：]\s*(.+)$",raw,re.I|re.S)
    if bm and not re.search(r"هر\s*قالب", bm.group(1), re.I):
        section=_assistant_clean_name(bm.group(1)); items=_assistant_bank_items(bm.group(2))
        if section and items: actions.append({"type":"add_bank","section":section,"items":items})

    # بانک توزیعی: مثلا «برای بخش ل، هر قالب ۱۰ تا: ۲۰۰ خط»
    dm=re.search(r"(?:برای\s+)?(?:بخش|قسمت)\s+[«\"']?([^،,:]+)[»\"']?\s*(?:،|,)?\s*(?:برای\s+)?(?:هر\s+قالب|هرقالب)\s*(?:[،,:\-]\s*)?(\d+|[آ-ی]+)\s*(?:تا|عدد|متن)?\s*(?:متن|عنوان|آیتم)?\s*[:：]\s*(.+)$",raw,re.I|re.S)
    if dm:
        per=_assistant_number(dm.group(2)); items=_assistant_bank_items(dm.group(3)); section=_assistant_clean_name(dm.group(1))
        if per and items: actions.append({"type":"distribute_bank","section":section,"per_template":per,"items":items})

    # بانک برای محدوده‌ای از قالب‌ها: «قالب‌های ۲ تا ۲۰ هر کدام ۱۰ تا»
    rd=re.search(r"(?:بخش\s+)?[«\"']?([^،:]+?)[»\"']?\s*(?:،\s*)?(?:قالب(?:های| ها|های)?)\s*(\d+)\s*تا\s*(\d+)\s*(?:،\s*)?(?:هر\s+قالب\s*)?(\d+)\s*(?:تا)?\s*(?:متن|عنوان|آیتم)\s*[:：]\s*(.+)$",raw,re.I|re.S)
    if rd:
        items=_assistant_bank_items(rd.group(5))
        actions.append({"type":"distribute_bank","section":_assistant_clean_name(rd.group(1)),"per_template":int(rd.group(4)),"items":items,"start":int(rd.group(2)),"end":int(rd.group(3))})

    # «۱۹ تای بعدی هم همین» ادامه آخرین توزیع
    if re.search(r"(?:\d+|[آ-ی]+)\s*(?:تای|تا)\s*(?:بعدی|باقی|باقیمانده).*(?:همین|مثل قبل|همان)",n,re.I):
        actions.append({"type":"continue_distribution"})

    # عنوان یک قالب
    st=re.search(r"(?:عنوان|اسم)\s+(?:قالب\s+)?[«\"']?([^:：]+?)[»\"']?\s*[:：]\s*(.+)$",raw,re.I|re.S)
    if st and "همه" not in st.group(1):
        actions.append({"type":"set_title","template":_assistant_clean_name(st.group(1)),"title":st.group(2).strip()})

    # عنوان همه قالب‌های بخش، هر خط برای یک قالب
    sta=re.search(r"(?:عنوان(?:های)?\s+همه|همه\s+عنوان(?:ها)?)\s+(?:بخش\s+)?[«\"']?([^:：]+?)[»\"']?\s*[:：]\s*(.+)$",raw,re.I|re.S)
    if sta:
        actions.append({"type":"set_titles","section":_assistant_clean_name(sta.group(1)),"titles":_assistant_title_lines(sta.group(2))})

    # فیلدهای قالب: فوتر / متن لینک / کلمه لینک / عنوان / هدر
    field_patterns={
        "footer": r"(?:فوتر|پایین\s*متن)\s+(?:قالب\s+)?[«\"']?([^:：]+?)[»\"']?\s*[:：]\s*(.+)$",
        "link_text": r"(?:متن\s+لینک|لینک\s*تکست)\s+(?:قالب\s+)?[«\"']?([^:：]+?)[»\"']?\s*[:：]\s*(.+)$",
        "linked_word": r"(?:کلمه\s+لینک|عبارت\s+لینک)\s+(?:قالب\s+)?[«\"']?([^:：]+?)[»\"']?\s*[:：]\s*(.+)$",
        "header": r"(?:هدر|سرمتن)\s+(?:قالب\s+)?[«\"']?([^:：]+?)[»\"']?\s*[:：]\s*(.+)$",
    }
    for field,pat in field_patterns.items():
        m=re.search(pat,raw,re.I|re.S)
        if m:
            actions.append({"type":"edit_template","template":_assistant_clean_name(m.group(1)),"field":field,"value":m.group(2).strip()})

    # متن رندوم یک قالب
    rh=re.search(r"(?:متن(?:های)?\s+رندوم|هدر(?:های)?\s+رندوم)\s+(?:برای\s+)?(?:قالب\s+)?[«\"']?([^:：]+?)[»\"']?\s*[:：]\s*(.+)$",raw,re.I|re.S)
    if rh:
        actions.append({"type":"random_headers","template":_assistant_clean_name(rh.group(1)),"items":_assistant_bank_items(rh.group(2))})

    # نقل قول قالب یا بخش
    bq=re.search(r"(?:نقل\s*قول|کوت)\s+(?:قالب\s+)?[«\"']?([^:：]+?)[»\"']?\s*(?::\s*(.+))?$",n,re.I)
    if bq and bq.group(1).strip() not in {"همه","برای همه"}:
        actions.append({"type":"quote_template","template":_assistant_clean_name(bq.group(1)),"text":(bq.group(2) or "").strip()})

    bqa=re.search(r"(?:نقل\s*قول|کوت)\s+(?:همه|تمام)\s+(?:قالب(?:ها)?\s+)?(?:بخش\s+)?[«\"']?([^:：]+?)[»\"']?\s*(?::\s*(.+))?$",n,re.I)
    if bqa:
        actions.append({"type":"quote_section","section":_assistant_clean_name(bqa.group(1)),"text":(bqa.group(2) or "").strip()})

    # پست سریع
    qm=re.search(r"پست\s*سریع\s*(?:بخش|برای|در|داخل)?\s*[«\"']?([^«»\"'،,؛;]+?)(?:\s+رو)?\s*(?:را\s*)?(فعال|روشن|خاموش|غیرفعال)",n,re.I)
    if qm:
        actions.append({"type":"quick","section":_assistant_clean_name(qm.group(1)),"enabled":qm.group(2) in ("فعال","روشن")})

    # ریست بانک
    rm=re.search(r"(?:ریست|بازنشانی)\s+(?:بانک|بانک\s+عنوان|بانک\s+قالب)\s*(?:بخش|برای|در|داخل)?\s*[«\"']?([^«»\"'،,؛;]+?)[»\"']?\s*$",n,re.I)
    if rm:
        actions.append({"type":"reset_bank","section":_assistant_clean_name(rm.group(1))})

    # کپی قالب N تا
    cp=re.search(r"(?:قالب\s+)?[«\"']?([^«»\"'،,;]+?)[»\"']?\s+(?:را\s+)?(?:به|برای)\s+(\d+|[آ-ی]+)\s*(?:تا|کپی)\s*(?:کپی)?(?:\s+با\s+نام\s+(.+))?",n,re.I)
    if cp and ("کپی" in low):
        actions.append({"type":"copy_template","source":_assistant_clean_name(cp.group(1)),"count":_assistant_number(cp.group(2)) or 1,"prefix":_assistant_clean_name(cp.group(3) or "")})

    # افزودن قالب موجود به بخش
    at=re.search(r"(?:قالب\s+)?[«\"']?([^»«\"'،,]+?)[»\"']?\s+(?:را\s+)?(?:به|داخل)\s+(?:بخش|پوشه)\s+[«\"']?([^»«\"'،,;]+?)[»\"']?\s+اضافه\s+کن",n,re.I)
    if at:
        actions.append({"type":"attach_template","template":_assistant_clean_name(at.group(1)),"section":_assistant_clean_name(at.group(2))})

    # حذف / پاک کردن (در اجرا تأیید می‌گیرد)
    ds=re.search(r"(?:بخش|پوشه)\s+[«\"']?([^»«\"'،,;]+?)[»\"']?\s+(?:را\s+)?(?:حذف|پاک)\s+کن",n,re.I)
    if ds: actions.append({"type":"delete_section","section":_assistant_clean_name(ds.group(1))})
    dt=re.search(r"(?:قالب)\s+[«\"']?([^»«\"'،,;]+?)[»\"']?\s+(?:را\s+)?(?:حذف|پاک)\s+کن",n,re.I)
    if dt: actions.append({"type":"delete_template","template":_assistant_clean_name(dt.group(1))})

    # موضوعات
    ta=re.search(r"(?:موضوع|تاپیک)\s+(?:به\s+نام\s+|با\s+نام\s+)?[«\"']?([^:：]+?)[»\"']?\s*[:：]\s*(.+)$",raw,re.I|re.S)
    if ta: actions.append({"type":"topic_add","name":_assistant_clean_name(ta.group(1)),"text":ta.group(2).strip()})
    td=re.search(r"(?:موضوع|تاپیک)\s+[«\"']?([^»«\"'،,;]+?)[»\"']?\s+(?:را\s+)?(?:حذف|پاک)\s+کن",n,re.I)
    if td: actions.append({"type":"topic_delete","name":_assistant_clean_name(td.group(1))})
    tv=re.search(r"(?:متن(?:های)?\s+جدید|تنوع|واریانت)\s+(?:موضوع|تاپیک)\s+[«\"']?([^:：]+?)[»\"']?\s*[:：]\s*(.+)$",raw,re.I|re.S)
    if tv: actions.append({"type":"topic_variant","name":_assistant_clean_name(tv.group(1)),"items":_assistant_bank_items(tv.group(2))})

    # ادمین‌ها
    aa=re.search(r"(?:ادمین|مدیر)\s+(?:با\s+)?(?:آیدی|id)?\s*(\d+)\s*(?:را\s+)?(?:اضافه|ادمین)\s+کن",n,re.I)
    if aa: actions.append({"type":"admin_add","id":aa.group(1)})
    ar=re.search(r"(?:ادمین|مدیر)\s+(?:با\s+)?(?:آیدی|id)?\s*(\d+)\s*(?:را\s+)?(?:حذف|برکنار)\s+کن",n,re.I)
    if ar: actions.append({"type":"admin_remove","id":ar.group(1)})
    af=re.search(r"(?:دسترسی\s+کامل|فول\s+اکسس)\s+(?:ادمین\s+)?(?:با\s+)?(?:آیدی|id)?\s*(\d+)\s*(روشن|خاموش|فعال|غیرفعال)",n,re.I)
    if af: actions.append({"type":"admin_full","id":af.group(1),"enabled":af.group(2) in ("روشن","فعال")})

    # نمایش/اجرای بخش‌های اصلی بات از داخل دستیار
    if re.search(r"^(?:وضعیت|آمار|گزارش|لیست بخش(?:ها)?|لیست قالب(?:ها)?)$",low):
        actions.append({"type":"status"})
    elif re.search(r"^(?:قالب(?:ها)?|مدیریت قالب(?:ها)?)$",low):
        actions.append({"type":"open","target":"templates"})
    elif re.search(r"^(?:لیست موضوع(?:ها)?|موضوعات)$",low):
        actions.append({"type":"open","target":"topics"})
    elif re.search(r"^(?:ادمین(?:ها)?|مدیریت ادمین(?:ها)?)$",low):
        actions.append({"type":"open","target":"admins"})
    elif re.search(r"^(?:پست(?:ها)?ی من|پست های من)$",low):
        actions.append({"type":"open","target":"myposts"})
    elif re.search(r"^(?:نهایی|پوشه(?:ها)?)$",low):
        actions.append({"type":"open","target":"final"})
    elif re.search(r"^(?:آماده(?:سازی)?|ساخت پست)$",low):
        actions.append({"type":"open","target":"ready"})
    elif re.search(r"^(?:پست سریع)$",low):
        actions.append({"type":"open","target":"quick"})
    elif re.search(r"^(?:بکاپ|پشتیبان)$",low):
        actions.append({"type":"backup"})
...
def _assistant_help_text():
    return (
        ...
        "📁 بخش و قالب:\n"
        "• بخش وطنی بساز\n"
        "• ۲۰ تا قالب برای وطنی با نام وطن بساز\n"
        "• قالب وطن_01 را به بخش ل اضافه کن\n"
        "• قالب وطن_01 را حذف کن\n\n"
        "🏷️ بانک و عنوان:\n"
        "• بانک بخش ل: متن۱ | متن۲ | متن۳\n"
        "• برای بخش ل، هر قالب ۱۰ تا متن: ۲۰۰ متن\n"
        "• عنوان وطن_01: عنوان جدید\n"
        "• عنوان‌های همه ل: هر خط برای یک قالب\n"
        "• ریست بانک ل\n"
        "• ۱۹ تای بعدی هم همین\n\n"
        "🧩 تنظیم قالب:\n"
        "• فوتر قالب وطن_01: متن فوتر\n"
        "• متن لینک قالب وطن_01: متن لینک\n"
        "• کلمه لینک قالب وطن_01: مشاهده\n"
        "• متن رندوم وطن_01: متن۱ | متن۲\n"
        "• نقل قول وطن_01: عبارت\n\n"
        "⚡ پست سریع:\n"
        "• پست سریع ل رو روشن کن\n\n"
        "🎬 موضوع:\n"
        "• موضوع وطنی: متن موضوع\n"
        "• تنوع موضوع وطنی: متن۱ | متن۲\n\n"
        "👮 ادمین (فقط مالک):\n"
        "• ادمین 123456789 اضافه کن\n"
        "• ادمین 123456789 حذف کن\n\n"
        "🛡️ حذف‌های خطرناک قبل از اجرا تأیید می‌گیرند.\n"
        "برای خروج: «خروج»"
    )

async def _assistant_execute(update,context,actions):
    uid=update.effective_user.id
    if not has_permission(uid,"manage_templates") and not is_owner(uid):
        await update.message.reply_text("⛔ برای استفاده از دستیار، دسترسی مدیریت قالب‌ها لازم است.")
        return True
    report=[]
    for a in actions:
        typ=a["type"]
        if typ=="help":
            await update.message.reply_text(_assistant_help_text(),reply_markup=get_main_keyboard(uid)); continue
        if typ=="exit":
            context.user_data.clear(); await update.message.reply_text("🤖 دستیار بسته شد.",reply_markup=get_main_keyboard(uid)); return True
        if typ=="open":
            target=a.get("target")
            if target=="templates": await template_manager_cmd(update,context)
            elif target=="topics": await update.message.reply_text("🎬 موضوعات را از بخش مدیریت موضوعات بات باز کن؛ دستیار آماده اجرای افزودن/حذف/تنوع موضوع هم هست.",reply_markup=get_main_keyboard(uid))
            elif target=="admins": await admin_menu_cmd(update,context)
            elif target=="myposts": await my_posts_cmd(update,context)
            elif target=="final": await final_menu_cmd(update,context)
            elif target=="ready": await ready_cmd(update,context)
            elif target=="quick": await quick_post_cmd(update,context)
            continue
        if typ=="backup":
            if is_owner(uid): await backup_cmd(update,context)
            else: report.append("⛔ بکاپ فقط برای مالک است.")
        elif typ=="create_section":
            ...
            if not has_permission(uid,"manage_templates"): report.append("⛔ اجازه ساخت قالب نداری."); continue
            ...
            save_texts(); save_template_groups(); save_title_banks()
            report.append(f"📦 {len(made)} قالب برای «{section}» ساخته شد." + (f"\n{', '.join(made[:20])}" if len(made)<=20 else f"\n{', '.join(made[:10])} ... {', '.join(made[-3:])}"))
        elif typ in {"add_bank","distribute_bank","continue_distribution"}:
            if typ=="continue_distribution":
                pending=context.user_data.get("assistant_distribution")
                if not pending: report.append("❌ توزیع قبلی برای ادامه پیدا نشد."); continue
                a=pending
            if typ=="add_bank":
                g=_assistant_find_section(a["section"]) or _assistant_section_key(a["section"])
                keys=[k for k in TEMPLATE_GROUPS.get(g,[]) if k in ALL_TEXTS]
                if not keys: report.append(f"❌ در بخش «{a['section']}» قالبی پیدا نشد."); continue
                for k in keys: add_title_bank_items(g,k,a["items"])
                report.append(f"🏦 {len(a['items'])} متن به بانک {len(keys)} قالب بخش «{a['section']}» اضافه شد.")
            else:
                g=_assistant_find_section(a["section"]) or _assistant_section_key(a["section"])
                keys=[k for k in TEMPLATE_GROUPS.get(g,[]) if k in ALL_TEXTS]
                start=int(a.get("start",1)); end=int(a.get("end",len(keys)))
                selected=keys[max(0,start-1):min(len(keys),end)]
                per=int(a.get("per_template",0)); items=list(a.get("items",[]))
                if not selected or per<=0: report.append("❌ بخش یا تعداد هر قالب نامعتبر است."); continue
                needed=len(selected)*per
                if len(items)==per:
                    chunks=[items for _ in selected]
                elif len(items)>=needed:
                    chunks=[items[i*per:(i+1)*per] for i in range(len(selected))]
                else:
                    report.append(f"❌ برای {len(selected)} قالب، {needed} متن لازم است؛ {len(items)} متن فرستادی."); continue
                for k,chunk in zip(selected,chunks): add_title_bank_items(g,k,chunk)
                last=start+len(selected)-1
                context.user_data["assistant_distribution"]={"section":a["section"],"per_template":per,"items":items,"start":last+1,"end":end}
                report.append(f"🏦 بانک {len(selected)} قالب پر شد؛ برای هر قالب {per} متن جداگانه ثبت شد.")
        elif typ=="set_title":
            k=_assistant_find_template(a["template"])
            if not k: report.append(f"❌ قالب «{a['template']}» پیدا نشد."); continue
            ALL_TEXTS[k]["title"]=a["title"].strip(); ALL_TEXTS[k]["header"]=""; ALL_TEXTS[k]["random_headers"]=[]; save_texts(); report.append(f"🏷️ عنوان «{k}» تنظیم شد.")
        elif typ=="set_titles":
            g=_assistant_find_section(a["section"]) or _assistant_section_key(a["section"]); keys=[k for k in TEMPLATE_GROUPS.get(g,[]) if k in ALL_TEXTS]; titles=a["titles"]
            changed=min(len(keys),len(titles))
            for i in range(changed): ALL_TEXTS[keys[i]]["title"]=titles[i]; ALL_TEXTS[keys[i]]["header"]=""; ALL_TEXTS[keys[i]]["random_headers"]=[]
            save_texts(); report.append(f"🏷️ عنوان {changed} قالب بخش «{a['section']}» تنظیم شد.")
        elif typ=="edit_template":
            k=_assistant_find_template(a["template"])
            if not k: report.append(f"❌ قالب «{a['template']}» پیدا نشد."); continue
            field=a["field"]; value=a["value"]
            if field=="linked_word" and value not in {"همه","کل متن"} and value not in str(ALL_TEXTS[k].get("link_text","")):
                report.append("❌ کلمه لینک داخل متن لینک نیست."); continue
            ALL_TEXTS[k][field]="" if value in {"همه","کل متن"} and field=="linked_word" else value
            save_texts(); report.append(f"✏️ {field} قالب «{k}» تغییر کرد.")
        elif typ=="random_headers":
            k=_assistant_find_template(a["template"])
            if not k: report.append(f"❌ قالب «{a['template']}» پیدا نشد."); continue
            arr=ALL_TEXTS[k].setdefault("random_headers",[])
            for x in a["items"]:
                if x not in arr: arr.append(x)
            save_texts(); report.append(f"🎲 {len(a['items'])} متن رندوم برای «{k}» ثبت شد.")
        elif typ=="quote_template":
            k=_assistant_find_template(a["template"])
            if not k: report.append(f"❌ قالب «{a['template']}» پیدا نشد."); continue
            ALL_TEXTS[k]["blockquote"]=True
            if a.get("text"): ALL_TEXTS[k]["quote_text"]=a["text"]
            save_texts(); report.append(f"💬 نقل‌قول «{k}» روشن شد.")
        elif typ=="quote_section":
            g=_assistant_find_section(a["section"]) or _assistant_section_key(a["section"]); keys=[k for k in TEMPLATE_GROUPS.get(g,[]) if k in ALL_TEXTS]
            if not keys: report.append(f"❌ بخش «{a['section']}» پیدا نشد."); continue
            qtext=a.get("text","");
            for k in keys:
                ALL_TEXTS[k]["blockquote"]=True
                if qtext: ALL_TEXTS[k]["quote_text"]=qtext
            set_section_last_quote(g,qtext); save_texts(); report.append(f"💬 نقل‌قول برای {len(keys)} قالب بخش «{a['section']}» روشن شد.")
        elif typ=="quick":
            g=_assistant_find_section(a["section"]) or _assistant_section_key(a["section"])
        ...
        elif typ=="copy_template":
            if not has_permission(uid,"manage_templates"): report.append("⛔ اجازه کپی قالب نداری."); continue
            source=_assistant_find_template(a["source"]); count=max(1,min(int(a["count"]),1000))
            if not source: report.append(f"❌ قالب «{a['source']}» پیدا نشد."); continue
            import copy as _copy
            groups=[g for g,vals in TEMPLATE_GROUPS.items() if source in vals]
            base=a.get("prefix") or (source+"_copy")
            names=[]; i=1
            while len(names)<count:
                candidate=f"{base}_{i:02d}"
                i+=1
                if candidate not in ALL_TEXTS: names.append(candidate)
            for name in names: ALL_TEXTS[name]=_copy.deepcopy(ALL_TEXTS[source])
            for g in groups:
                vals=TEMPLATE_GROUPS[g]; pos=vals.index(source)+1; vals[pos:pos]=names
                for name in names: _title_bank_entry(g,name)
            save_texts(); save_template_groups(); save_title_banks(); report.append(f"📄 {len(names)} کپی از «{source}» ساخته شد.")
        elif typ=="attach_template":
            k=_assistant_find_template(a["template"]); g=_assistant_find_section(a["section"]) or _assistant_section_key(a["section"])
            if not k: report.append(f"❌ قالب «{a['template']}» پیدا نشد."); continue
            if g not in TEMPLATE_GROUPS: report.append(f"❌ بخش «{a['section']}» پیدا نشد."); continue
            if k not in TEMPLATE_GROUPS[g]: TEMPLATE_GROUPS[g].append(k)
            _title_bank_entry(g,k); save_template_groups(); save_title_banks(); report.append(f"📦 قالب «{k}» به بخش «{a['section']}» اضافه شد.")
        elif typ in {"delete_section","delete_template","topic_delete","admin_remove"}:
            # تأیید دو مرحله‌ای برای حذف‌های واقعی
            context.user_data["assistant_pending_delete"]=a
            label=(f"بخش «{a['section']}»" if typ=="delete_section" else f"قالب «{a['template']}»" if typ=="delete_template" else f"موضوع «{a['name']}»" if typ=="topic_delete" else f"ادمین {a['id']}")
            await update.message.reply_text(f"⚠️ مطمئنی {label} حذف شود؟\n\nبرای تأیید بنویس: بله\nبرای لغو: خیر")
            return True
        elif typ=="topic_add":
            if not has_permission(uid,"manage_topics"): report.append("⛔ اجازه مدیریت موضوعات نداری."); continue
            TOPICS[a["name"]]=a["text"]
            save_texts(); report.append(f"🎬 موضوع «{a['name']}» ذخیره شد.")
        elif typ=="topic_variant":
            if not has_permission(uid,"manage_topics"): report.append("⛔ اجازه مدیریت موضوعات نداری."); continue
            TOPIC_VARIANTS[a["name"]]=a["items"]; save_json(TOPIC_VARIANTS_FILE,TOPIC_VARIANTS); report.append(f"🎲 {len(a['items'])} تنوع برای موضوع «{a['name']}» ثبت شد.")
        elif typ=="admin_add":
            if not is_owner(uid): report.append("⛔ فقط مالک می‌تواند ادمین اضافه کند."); continue
            ADMINS.setdefault(str(a["id"]),{"full_access":False,"permissions":[]}); save_admins(); report.append(f"👮 ادمین {a['id']} اضافه شد.")
        elif typ=="admin_full":
            if not is_owner(uid): report.append("⛔ فقط مالک می‌تواند دسترسی ادمین را تغییر دهد."); continue
            aid=str(a["id"])
            if aid not in ADMINS: report.append("❌ ادمین پیدا نشد."); continue
            ADMINS[aid]["full_access"]=a["enabled"]; save_admins(); report.append(f"👮 دسترسی کامل ادمین {aid} {'روشن' if a['enabled'] else 'خاموش'} شد.")
        elif typ=="status":
            sections=sum(1 for g,v in TEMPLATE_GROUPS.items() if v)
            report.append(f"📊 وضعیت\n\n📁 بخش‌ها: {sections}\n🧩 قالب‌ها: {len(ALL_TEXTS)}\n🎬 موضوع‌ها: {len(TOPICS)}\n👮 ادمین‌ها: {len(ADMINS)}")

    if report:
        await update.message.reply_text("🤖 انجام شد.\n\n"+"\n".join(report),reply_markup=get_main_keyboard(uid))
    return True
...
async def assistant_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not can_use_bot(uid):
        ...
        return
    if not has_permission(uid, "manage_templates") and not is_owner(uid):
        await update.message.reply_text("⛔ برای استفاده از دستیار، دسترسی مدیریت قالب‌ها لازم است.",reply_markup=get_main_keyboard(uid)); return
    context.user_data.clear(); context.user_data["assistant_mode"] = True
    await update.message.reply_text(
        "🤖 دستیار کامل آماده است!\n\n"
        "تقریباً همه کارهای مدیریتی بات را می‌توانی با جمله فارسی به من بگویی.\n"
        "مثال: «۲۰ تا قالب برای بخش ل با نام وطن بساز»\n"
        "یا: «برای بخش ل، هر قالب ۱۰ تا متن: ...»\n\n"
        "برای راهنمای کامل: «کمک»\nبرای خروج: «خروج»",
        reply_markup=get_main_keyboard(uid))
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not can_use_bot(uid):
        await update.message.reply_text("⛔ دسترسی نداری!")
        return
    # تأیید حذف‌های خطرناک دستیار
    if context.user_data.get("assistant_pending_delete") and text.strip().lower() in {"بله", "بله حتما", "yes", "تایید", "تأیید"}:
        pending = context.user_data.pop("assistant_pending_delete")
        if pending.get("type") == "delete_template":
            k = _assistant_find_template(pending.get("template", ""))
            if k:
                for g in list(TEMPLATE_GROUPS):
                    TEMPLATE_GROUPS[g] = [x for x in TEMPLATE_GROUPS[g] if x != k]
                ALL_TEXTS.pop(k, None)
                save_texts(); save_template_groups()
                for g in list(TITLE_BANKS):
                    TITLE_BANKS[g].pop(k, None)
                save_title_banks()
                await update.message.reply_text(f"🗑 قالب «{k}» حذف شد.", reply_markup=get_main_keyboard(uid))
            else: await update.message.reply_text("❌ قالب دیگر وجود ندارد.", reply_markup=get_main_keyboard(uid))
        elif pending.get("type") == "delete_section":
            g = _assistant_find_section(pending.get("section", ""))
            if g:
                keys=list(TEMPLATE_GROUPS.get(g,[]))
                TEMPLATE_GROUPS.pop(g,None); SECTION_SETTINGS.pop(g,None); TITLE_BANKS.pop(g,None)
                save_template_groups(); save_section_settings(); save_title_banks()
                await update.message.reply_text(f"🗑 بخش «{section_title(g)}» حذف شد؛ {len(keys)} قالب خودشان حذف نشدند.", reply_markup=get_main_keyboard(uid))
            else: await update.message.reply_text("❌ بخش دیگر وجود ندارد.", reply_markup=get_main_keyboard(uid))
        elif pending.get("type") == "topic_delete":
            name=pending.get("name",""); TOPICS.pop(name,None); TOPIC_VARIANTS.pop(name,None); save_texts(); save_json(TOPIC_VARIANTS_FILE,TOPIC_VARIANTS)
            await update.message.reply_text(f"🗑 موضوع «{name}» حذف شد.", reply_markup=get_main_keyboard(uid))
        elif pending.get("type") == "admin_remove":
            if not is_owner(uid): await update.message.reply_text("⛔ فقط مالک می‌تواند ادمین حذف کند.")
            else:
                aid=str(pending.get("id")); existed=ADMINS.pop(aid,None); save_admins()
                await update.message.reply_text((f"🗑 ادمین {aid} حذف شد." if existed else "❌ ادمین پیدا نشد."), reply_markup=get_main_keyboard(uid))
        return
    if context.user_data.get("assistant_pending_delete") and text.strip().lower() in {"خیر", "نه", "لغو", "cancel", "no"}:
        context.user_data.pop("assistant_pending_delete", None)
        await update.message.reply_text("❌ حذف لغو شد.", reply_markup=get_main_keyboard(uid))
        return
